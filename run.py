...
def main():
    ...
    parser.add_argument('--entity_sent', action='store_true')
    parser.add_argument('--caption_sent',action='store_true')
    parser.add_argument('--use_entity_cos',action='store_true')
    ...
    if args.save_path is not None:  # make save_path dir
        ...
        args.save_path = os.path.join(args.save_path, args.dataset_name+"_"+str(args.batch_size)+"_"+str(args.lr)+"_"+args.notes + ("_entity" if args.entity else '') +("_caption" if args.caption else ''))
    ...
    writer=None
    filename = 'logging/'+ "_" + str(datetime.datetime.now())+'_'+args.dataset_name+ "_"+str(args.batch_size) + "_" + str(args.lr) + args.notes 
    if args.entity:
        filename += '_entity'
    if args.caption:
        filename += '_caption'
    ...
    train_dataset = dataset_class(processor, transform, args,img_path, aux_path, args.max_seq, sample_ratio=args.sample_ratio, mode='train')
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)

    dev_dataset = dataset_class(processor, transform,args, img_path, aux_path, args.max_seq, mode='dev')
    dev_dataloader = DataLoader(dev_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)

    test_dataset = dataset_class(processor, transform,args, img_path, aux_path, args.max_seq, mode='test')
