...
class RETrainer(BaseTrainer):
    ...
    def _step(self, batch, mode="train"):
        if mode != "predict":
            if self.args.use_prompt:
                input_ids, token_type_ids, attention_mask, labels, images, aux_imgs ,cap_input_ids,cap_token_type_ids,cap_attention_mask, entities,ent_input_ids , ent_token_type_ids ,ent_attention_mask= batch
            else:
                images, aux_imgs = None, None
                input_ids, token_type_ids, attention_mask, labels ,cap_input_ids,cap_token_type_ids,cap_attention_mask, entities ,ent_input_ids , ent_token_type_ids ,ent_attention_mask= batch
            outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, labels=labels,entities = entities, images=images, aux_imgs=aux_imgs,
                                    cap_input_ids = cap_input_ids,cap_token_type_ids = cap_token_type_ids,cap_attention_mask=cap_attention_mask,
                                    ent_input_ids = ent_input_ids,ent_token_type_ids = ent_token_type_ids,ent_attention_mask=ent_attention_mask)
...
class NERTrainer(BaseTrainer):
    ...
    def _step(self, batch, mode="train"):
        if self.args.use_prompt:
            input_ids, token_type_ids, attention_mask, labels, images, aux_imgs ,entities = batch
        else:
            images, aux_imgs = None, None
            input_ids, token_type_ids, attention_mask, labels = batch
        output = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, entities = entities ,labels=labels, images=images, aux_imgs=aux_imgs)
