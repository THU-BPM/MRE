...
class MMREProcessor(object):
    ...
    def load_from_file(self, mode="train", sample_ratio=1.0):
        ...
        with open(load_file, "r", encoding="utf-8") as f:
            lines = f.readlines()
            words, relations, heads, tails, imgids, dataid, captions,entities  = [], [], [], [], [], [], [] , []
        ...
        return {'words':words, 'relations':relations, 'heads':heads, 'tails':tails, 'imgids': imgids, 'dataid': dataid, 'aux_imgs':aux_imgs, 'captions':captions,'entities':entities}
...
class MMREDataset(Dataset):
    def __init__(self, processor, transform, args,img_path=None, aux_img_path=None, max_seq=40, sample_ratio=1.0, mode="train") -> None:
    ...
    def __getitem__(self, idx):
        ...
        captions = self.data_dict['captions'][idx]
        entities = self.data_dict['entities'][idx]
        entities = [entitie.replace('|','') for entitie in entities]
        if len(entities) < 11:
            index = 11-len(entities)
        ...
        # cap_input_ids , cap_token_type_ids ,cap_attention_mask = [], [], []
        # for caption in captions:
        #     encode_dict = self.tokenizer.encode_plus(text=caption, max_length=self.max_seq, truncation=True, padding='max_length')
        #     cap_input_ids.append(encode_dict['input_ids'])
        #     cap_token_type_ids.append(encode_dict['token_type_ids'])
        #     cap_attention_mask.append(encode_dict['attention_mask'])
        if captions is not None:
            if self.args.caption_sent:
                encode_dict = self.tokenizer.encode_plus(text=' '.join(captions), max_length=self.max_seq, truncation=True, padding='max_length')
            else:
                encode_dict = self.tokenizer.encode_plus(text=captions[0], max_length=self.max_seq, truncation=True, padding='max_length')
            cap_input_ids = encode_dict['input_ids']
            cap_token_type_ids = encode_dict['token_type_ids']
            cap_attention_mask = encode_dict['attention_mask']
            cap_input_ids , cap_token_type_ids ,cap_attention_mask = torch.tensor(cap_input_ids),torch.tensor(cap_token_type_ids),torch.tensor(cap_attention_mask)
            
        entity_sent = entities.replace('|', ' ')
        if self.args.use_entity_cos:
            entity_sent = self.data_dict['entities'][idx][0]
        encode_dict = self.tokenizer.encode_plus(text=entity_sent, max_length=self.max_seq, truncation=True, padding='max_length')
        ent_input_ids = encode_dict['input_ids']
        ent_token_type_ids = encode_dict['token_type_ids']
        ent_attention_mask = encode_dict['attention_mask']
        ent_input_ids , ent_token_type_ids ,ent_attention_mask = torch.tensor(ent_input_ids),torch.tensor(ent_token_type_ids),torch.tensor(ent_attention_mask)
        ...
        if self.img_path is not None:
            ...
            return input_ids, token_type_ids, attention_mask, torch.tensor(re_label), image, aux_imgs,cap_input_ids,cap_token_type_ids,cap_attention_mask,entities,ent_input_ids , ent_token_type_ids ,ent_attention_mask
        return input_ids, token_type_ids, attention_mask, torch.tensor(re_label) ,cap_input_ids,cap_token_type_ids,cap_attention_mask,entities,ent_input_ids , ent_token_type_ids ,ent_attention_mask
